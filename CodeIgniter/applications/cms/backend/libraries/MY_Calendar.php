...
class MY_Calendar
{
    ...
    *********************************************************************************/
    ...
    function getMonthHTML($m, $y, $showYear = 1)
    {
        ...
        $s .= "<div class=\"calendario\">";
        $s .= "<div>\n";
        $s .= "<span id=\"lado\">" . (($prevMonth == "") ? "&nbsp;" : "<a href=\"$prevMonth\">&nbsp;<&nbsp;</a>")  . "</span>\n";
        $s .= "<span  class=\"month\" colspan=\"5\">$header</span>\n";
        $s .= "<span id=\"lado\">" . (($nextMonth == "") ? "&nbsp;" : "<a href=\"$nextMonth\">&nbsp;>&nbsp;</a>")  . "</span>\n";
        $s .= "</div>\n";
        $s .= "<br />\n";

        $s .= "<div>\n";
        $s .= "<span  class=\"calendarHeader\">&nbsp;" . $this->dayNames[($this->startDay)%7] . "</span>\n";
        $s .= "<span  class=\"calendarHeader\">&nbsp;" . $this->dayNames[($this->startDay+1)%7] . "</span>\n";
        $s .= "<span  class=\"calendarHeader\">&nbsp;" . $this->dayNames[($this->startDay+2)%7] . "</span>\n";
        $s .= "<span  class=\"calendarHeader\">&nbsp;" . $this->dayNames[($this->startDay+3)%7] . "</span>\n";
        $s .= "<span  class=\"calendarHeader\">&nbsp;" . $this->dayNames[($this->startDay+4)%7] . "</span>\n";
        $s .= "<span  class=\"calendarHeader\">&nbsp;" . $this->dayNames[($this->startDay+5)%7] . "</span>\n";
        $s .= "<span  class=\"calendarHeader\">&nbsp;" . $this->dayNames[($this->startDay+6)%7] . "</span>\n";
        $s .= "</div>\n";
        $s .= "<br />";
        ...
        while ($d <= $daysInMonth)
        {
            ...
            for ($i = 0; $i < 7; $i++)
            {
                ...
                if ($d > 0 && $d <= $daysInMonth)
                {
                    ...
                    if($d < 10)
                    {
                        $s .= (($link == "") ? $d : '<a id="day_agenda"  href="'.$link.'" >&nbsp;'.$d.'</a>');
                    }else{
                        $s .= (($link == "") ? $d : '<a id="day_agenda"  href="'.$link.'" >'.$d.'</a>');
                    }
                }
