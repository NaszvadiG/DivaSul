...
class Imagens extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->module_url = site_url('site/'.$this->router->class).'/';
    }
    ...
    function index()
    {
        ...
        // Aqui obtem os parametros da funcao
        $args = array();
        ...
        // Diretorio /arquivos
        $arquivos_path = $this->limpar_barras(realpath(SERVERPATH.$this->site_dir.'/arquivos/newsletter').'/');
        $arquivos_url = '/';
        if ( strlen($this->site_dir) > 0 ) { $arquivos_url .= $this->site_dir.'/'; }
        $arquivos_url .= 'arquivos/newsletter/';
        $current_dir = $this->limpar_barras(implode('/',$args).'/');
        if ( $current_dir == '/' ) { $current_dir = ''; } // Se esta em /arquivos, nao precisa desta barra dupla :)

        // Define umas variaveis para a view
        ...
        // Navbar (breadcrumbs)
        $dados['breadcrumbs'] = '';
        $dados['breadcrumbs'] .= '<a href="'.$this->module_url.'">arquivos / newsletter</a>';
        if ( !empty($args) )
        {
            foreach ( $args as $item )
            {
                ...
                // NÃ£o deixa procurar pastas anteriores a raiz
        ...
        // Armazena na sessao o diretorio atual
        ...
        if ( $_POST['action'] == 'novapasta' )
        {
            ...
            {
                $dados['txt'] = 'Não foi possí­vel criar o diretório: <b>'.$novapasta.'</b>.';
            }
        }
        ...
        // Envia arquivos (nao e mais utilizado, agora tem o "multiupload")
        if ( $_FILES['arquivo'] )
        {
        ...
        $dados['dirs'] = $dirs;
        $dados['arquivos_path'] = $arquivos_path.$current_dir.'/';
    ...
    function editar()
    {
        ...
        if ( !empty($dados['name']) )
        {
            ...
            $this->load->view('imagens_editar', $dados);
        }
    }

    /**
     * Remover arquivos (aÃ§Ã£o do botÃ£o "Excluir")
     */
